...
public class CollisionPD2 : MonoBehaviour
{
    public enum PdInputMode
    {
        EnvelopeFollower,
        PitchTracking,
        Trigger
    ...
    [Header("Pure Data Audio Receiver Setup")]
    [SerializeField] private PdAudioReceiver audioReceiver;
    [SerializeField] private PdInputMode currentPdMode = PdInputMode.EnvelopeFollower;

    // 1. DICTIONARY: Ordnet dem Modus den LibPD-Receivernamen zu
    private readonly Dictionary<PdInputMode, string> pdReceiverMap = new Dictionary<PdInputMode, string>()
    {
        { PdInputMode.EnvelopeFollower, "envelope_follow" },
        { PdInputMode.PitchTracking,    "pitch_tracking"  },
        { PdInputMode.Trigger,          "trigger"         }
    };

    // 2. DICTIONARY: Ordnet dem Modus direkt die Rechenformel zu (Func<float, float> nimmt wert entgegen und gibt berechnetes Tempo zurück)
    private Dictionary<PdInputMode, Func<float, float>> speedCalculationMap;
    ...
    void Awake()
    {
        ...
        // Dictionary für die Formeln initialisieren
        InitializeSpeedCalculators();
    }
    ...
    private void InitializeSpeedCalculators()
    {
        speedCalculationMap = new Dictionary<PdInputMode, Func<float, float>>()
        {
            // Envelope Follower (Lautstärke): Wurzel für hohe Sensitivität bei leisen Tönen
            { 
                PdInputMode.EnvelopeFollower, 
                val => Mathf.Lerp(minSpeed, maxSpeed, Mathf.Sqrt(Mathf.Clamp01(val))) 
            },

            // Pitch Tracking (Tonhöhe): Quadratische Kurve für exponentielle Frequenzen
            { 
                PdInputMode.PitchTracking, 
                val => Mathf.Lerp(minSpeed, maxSpeed, Mathf.Pow(Mathf.Clamp01(val), 2.0f)) 
            },

            // Trigger (Schwellenwert/Impuls): Binarer Schalter
            { 
                PdInputMode.Trigger, 
                val => (val > 0.5f) ? maxSpeed : minSpeed 
            }
        };
    }

    // 3. ELEGANTE VERARBEITUNG OHNE SWITCH-CASE
    private void OnPdAudioReceived(float value)
    {
        // Wir schlagen die passende Formel im Dictionary nach und führen sie direkt aus
        if (speedCalculationMap.TryGetValue(currentPdMode, out Func<float, float> calculateSpeed))
        {
            float calculatedSpeed = calculateSpeed(value);
